 ...
 * 取消订单 */
@Controller
public class QuxiaodingdanController extends BaseController
{
    @Autowired
    private QuxiaodingdanMapper dao;
    @Autowired
    private QuxiaodingdanService service;
     ...
     *
     */
    @RequestMapping("/quxiaodingdan_list")
    public String list()
    {
        ...
        Example example = new Example(Quxiaodingdan.class); //  创建一个扩展搜索类
        ...
        List<Quxiaodingdan> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
    ...
    public String getWhere()
    {
        ...
        if(!Request.get("jiaoyishijian_end").equals("")) {
            ...
            where += " AND quxiaoyuanyin LIKE '%"+Request.get("quxiaoyuanyin")+"%' ";
        }
     ...
     * 卖家列表
     */
    @RequestMapping("/quxiaodingdan_list_maijia")
    public String listmaijia()
    {
        ...
        page = Math.max(1 , page); // 注释同list
            ...
            List<Quxiaodingdan> list = service.selectPageExample(example , page , pagesize);
                    assign("totalCount" , request.getAttribute("totalCount"));
        assign("list" , list);
     ...
     * 下单人列表
     */
    @RequestMapping("/quxiaodingdan_list_xiadanren")
     ...
     * 取消人列表
     */
    @RequestMapping("/quxiaodingdan_list_quxiaoren")
    ...
    @RequestMapping("/quxiaodingdan_add")
    public String add()
    {
        _var = new LinkedHashMap(); // 重置数据
    ...
    @RequestMapping("/quxiaodingdan_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Quxiaodingdan mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
        return json();   // 将数据写给前端
    }
     ...
     * @return
     */
    @RequestMapping("/quxiaodingdaninsert")
    public String insert()
    {
        _var = new LinkedHashMap(); // 重置数据
        String tmp="";
        Quxiaodingdan post = new Quxiaodingdan();  // 创建实体类
        // 设置前台提交上来的数据到实体类中
        ...
        Query.execute("update wupindingdan set dingdanzhuangtai='已取消' where id='"+request.getParameter("wupindingdanid")+"'");
        Query.execute("update wupinxinxi set wupinzhuangtai='在售' where wupinbianhao='"+request.getParameter("wupinbianhao")+"'");
    ...
    * @return
    */
    @RequestMapping("/quxiaodingdanupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Quxiaodingdan post = new Quxiaodingdan();
        // 将前台表单数据填充到实体类
     ...
     *  后台详情
     */
    @RequestMapping("/quxiaodingdan_detail")
    public String detail()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Quxiaodingdan map = service.find(id);  // 根据前台url 参数中的id获取行数据
    ...
    *  删除
    */
    @RequestMapping("/quxiaodingdan_delete")
    public String delete()
    {
        ...
        HashMap map = Query.make("quxiaodingdan").find(id);
